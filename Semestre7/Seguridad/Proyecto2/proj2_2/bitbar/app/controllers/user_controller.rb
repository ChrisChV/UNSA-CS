class UserController < ApplicationController
  ...
  def transfer
    ...
    session[:secretT] = SecureRandom.hex(32)
    @user = params[:user]
  ...
  def post_transfer(failure_form=:transfer_form)
	...
	elsif !params[:iToken] || params[:iToken] != session[:secretT]
      @error = "Session Error"
	end
  ...
  def view_profile
    ...
    if @username != nil
	    @username = @username.gsub('<script>','')
	    @username = @username.gsub('</script>','')
	    @username = @username.gsub('XMLHttpRequest','')
	    @username = @username.gsub('cookie','')
	    @username = @username.gsub('document','')
	    @username = @username.gsub(';','')
	    @username = @username.gsub('GET','')
	    @username = @username.gsub('POST','')
	end
  ...
  def set_profile
    ...
    profile = params[:new_profile]
    if profile != nil
    	profile = profile.gsub('XMLHttpRequest','')
    	profile = profile.gsub('.send(','')
    	profile = profile.gsub('.open(','')
    	profile = profile.gsub('bitbar_count','')
    	profile = profile.gsub('document.getElementById(','')
    end
