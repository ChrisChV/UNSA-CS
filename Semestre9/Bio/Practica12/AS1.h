...
class AS{
	public:
		AS(){}
		AS(matrix & costos, double femIni, double alfa, double delta, double Q ,double evaporacion, double E
			, int numH, int numIter);
		void run();
		matrix costos;
		matrix distancias;
		matrix feromonas;
...
AS::AS(matrix & costos, double femIni, double alfa, double delta, double Q ,double evaporacion, double E, 
			int numH, int numIter){
	this->costos = costos;
...
void AS::run(){
	...
	for(int i = 0; i < numIter; i++){
		...
		for(auto iter = mejorGlobal->res.begin(); iter != mejorGlobal->res.end(); ++iter){
			...
			newFeromonas[estadoAns][(*iter)] += mejorGlobal->ferCosto * E;
