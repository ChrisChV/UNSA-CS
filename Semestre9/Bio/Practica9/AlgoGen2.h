...
typedef function<void(vector<ParCromo>, Poblacion &)> AlgoGenCrossFunc; //hacer push
typedef function<void(Poblacion &)> AlgoGenMutFunc;
...
class AlgoGen{
	public:
		AlgoGen(int tamP, int tamCrom, int numIter, int lambda,
...
AlgoGen::AlgoGen(int tamP, int tamCrom, int numIter, int lambda,
				AlgoGenFitnessFunc fitness, AlgoGenSelectFunc select, AlgoGenCrossFunc cross, AlgoGenMutFunc mut,
				AlgoGenNextPoblFunc getNextPobl){
	this->tamP = tamP;
	this->tamCrom = tamCrom;
...
void AlgoGen::run(){
	...
	for(int i = 0; i < numIter + 1; i++){
		...
		cout<<endl;
		if(i == numIter) break;
		padres = select(pobl, lambda);
		newPobl.clear();
		cross(padres, newPobl);
		mut(newPobl);
