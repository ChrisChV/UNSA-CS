...
class Vector3 {
  public:
    ...
    Vector3 operator+(Vector3); // Suma
    Vector3 operator-(Vector3); // Resta
...
Vector3 Vector3::unitario(){
    vsalor mod = modulo();
    return Vector3(this->x/mod,this->y/mod,this->z/mod);
}
...
auto Vector3::operator+(Vector3 v2)
{
  return Vector3(this->x+v2.getX(),this->y+v2.getY(),this->z+v2.getZ());
}
