...
class GramaticaRegular
{
    ...
    private:
     ...
     bool _reconocerIdentificador(string cadena);
     bool _reconocerNumero(string cadena);
     bool _reconocerNoTerminal(string cadena);
};

bool GramaticaRegular::_reconocerNoTerminal(string cadena){
    int estado = 0;
...
bool GramaticaRegular::_reconocerNumero(string cadena){
    for(auto iter = cadena.begin(); iter != cadena.end(); ++iter){
...
bool GramaticaRegular::_reconocerIdentificador(string cadena){
    int estado = 0;
    for(auto iter = cadena.begin(); iter != cadena.end(); ++iter){
        switch(estado){
            case 0:
                if(isdigit(*iter)) return false;
                else if(isalpha(*iter) or *iter == '_') estado = 1;
                else return false;
                break;
            case 1:
                if(isdigit(*iter) or isalpha(*iter) or *iter == '_') estado = 1;
                else return false;
                break;
        }
    }
